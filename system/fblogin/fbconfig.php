<?php
    ...
    $helper = new FacebookRedirectLoginHelper($config['hotelUrl'].'/system/fblogin/fbconfig.php');
	try {
	...
	if ( isset( $session ) ) {
		...
		if ($config['facebookLogin'] == true)
		{
			if (DB::NumRowsQuery("SELECT fbid FROM users WHERE fbid = '".$_SESSION['FBID']."'") == 0)
			{
				...
				");
				$newUser = DB::Query("SELECT * FROM `users` WHERE username='FB_".DB::Escape(filter($_SESSION['FBID']))."' && mail = '".DB::Escape($_SESSION['EMAIL'])."' LIMIT 1");
				while ($User = $newUser->fetch_assoc())
				{
				...
				while ($User = $loadUser->fetch_assoc())
				{
					$_SESSION['id'] = DB::Escape($User['id']);
